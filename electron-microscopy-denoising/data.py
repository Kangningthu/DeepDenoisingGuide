...
def get_transform(training, rotation_aug, resize_aug, image_size):
    ...
    if training:
        ...
        transform.append(transforms.CenterCrop(image_size)) # Crop to 128x128 pixels


        
#    transform.append( transforms.functional.resize(x,
...
def get_transform_resized(image_size):
...
#        image_size)) if training else None
    ...
    transform.append( lambda x: transforms.functional.resize(x,
                                            (image_size, image_size),
                                            Image.BILINEAR))
    transform.append(transforms.ToTensor())
    return transforms.Compose(transform)
...
def load_ptceo2_v2(
    ...
    test_loader = torch.utils.data.DataLoader(
        test_dataset, batch_size=test_batch_size, num_workers=0, shuffle=False)
    return train_loader, valid_loader, test_loader



def load_ptceo2_v2_resized(
        data,
        contrast='white',
        batch_size=100,
        image_size=400,
    ...
    train_dataset = ParticleDataset(
        os.path.join(
            ...
            f'train_{contrast}.csv'),
        root_dir,
        get_transform_resized( image_size=image_size),
        generalization_exp = generalization_exp,
        allowed_gen_values = allowed_gen_values)
    train_dataset = torch.utils.data.ConcatDataset(
        [train_dataset] * repeat_train)
...
@register_dataset("ptceo2_resized")
def load_regular_dataloaders(
        data,
        contrast='white',
        batch_size=100,
    ...
    return load_ptceo2_v2_resized(data = data, contrast = contrast, batch_size = batch_size,
                            image_size = image_size, num_workers = num_workers, repeat_train = repeat_train,
                            test_batch_size = test_batch_size, rotation_aug = rotation_aug, resize_aug = resize_aug)
